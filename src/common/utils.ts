...
import { objectArrayToObjectEndPoint, AirtableRecord, flattenFieldsEndPoint } from './types';
...
export const hasFields = (value: any): boolean => exists(value.fields)
 ...
 */
export const pickTruthy = (): (obj: object) => object => pickBy(identity)
 ...
 */
export const mapValuesFlat = (obj: object) => {
  return flatMap(obj, (value: any) => {
 ...
 */
export const mapValuesDeep = (fn: Function, key?: any): (obj: object | object[]) => object => (obj: object | object[]) => 
  isArray(obj)
    ? map(
        (innerObj: object, idx: any) => mapValuesDeep(fn, idx)(innerObj)
      )(obj)
    : isPlainObject(obj)
      ? mapValues(
          (val:any, key:any) => mapValuesDeep(fn, key)(val)
        )(obj)
      : isObject(obj)
        ? obj
        : fn(obj, key)
 ...
 */
export const mapKeysDeep = (fn: any): (obj: object | object[]) => object => (obj: object | object[]) => 
  isArray(obj)
    ? map(
        (innerObj: object) => mapKeysDeep(fn)(innerObj) 
      )(obj)
    : isPlainObject(obj)
      ? flow([mapKeys(fn), mapValues(
          (value: any) => mapKeysDeep(fn)(value)
        )])(obj)
      : obj
 ...
 */
export function renameKeys(oldKey: string, newKey: string): (obj: object) => object{
  return (originalObj: any) => {
    if(has(oldKey)) {
 ...
 */
export function mergeIfExists(newData: any): (data: any) => any {
    return (data: any) => cond([[exists, merge(data)]])(newData)
}
 ...
 */
export const isRecordId = (value: any) => {
...
export const flattenFields = flattenFieldsEndPoint.validate((record) => {
  ...
  Object.entries(record.fields).map(([key, value]) => {
    if (Array.isArray(value) && hasFields(value[0])) {
      return value.map((subRecord: AirtableRecord) => {
 ...
 */
export const objectArrayToObject = objectArrayToObjectEndPoint.validate((args: any) => {
  return transform(
    args,
    (result: any, value: any, key: any) => {
      const realKey: any = first(keys(value))
