...
import { isBooleanTrueString, isBooleanFalseString, isBooleanNumber, isBooleanArrayString, isNumberArrayString, isStringArrayString, isNumberString, isObjectString } from './typeChecks';
/**
 * Boolean
 */
export const coerceBoolean = cond([
    [isBoolean, data => Boolean(data)],
    [isBooleanTrueString, stubTrue],
    [isBooleanFalseString, stubFalse],
    [isBooleanNumber, isBooleanNumber]
]);
export const coerceBooleanArray = map(coerceBoolean);
/**
 * String
 */
export const coerceString = cond([
    [isString, data => escape(data)]
]);
export const coerceStringArray = map(coerceString);
/**
 * Number
 */
export const coerceNumber = cond([
    [isNumber, data => parseFloat(data)],
    [isNumberString, data => parseFloat(data)]
]);
export const coerceNumberArray = map(coerceNumber);
/**
 * Object
 */
export const coerceObject = cond([
    [isObject, data => Object(data)],
    [isObjectString, data => yaml.parse(data)]
]);
export const coerceObjectArray = map(coerceObject);
...
const _coerceBooleanArray = coerceArray(z.boolean());
const test = _coerceBooleanArray('[true,false]');
// take 2
const coerceArrayAgain = (arrayType) => {
    return z
        .transformer(z.union([arrayType, z.array(arrayType)]), z.array(arrayType), n => cond([
