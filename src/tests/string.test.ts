import { zToString, zToStringArray } from '../string'
...
test.each([
  ...
  try {
    expect(zToString.parse(input))
  } catch (err) {
    expect(err.errors[0].message).toBe(
      'Value can be coerced to a type - consider updating to using the corresponding transform.'
    )
  }
})

test('Transformer: zToStringArray parses comma seperated strings with no spaces', () => {
  const uut = zToStringArray.parse('hello,bob,its,me')
